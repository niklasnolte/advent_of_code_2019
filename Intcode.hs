...
get_values_by_mode com modes n =
  [ get_val v mode
  | (v, mode) <- zip (map (\i -> getVal i seq) [start .. (n + start - 1)])
                     (modes ++ repeat Position)
  ]
 where
  start = _current_position com + 1
  seq   = _sequence com
...
perform_opcode_1or2 com modes op = com
  { _sequence         = M.insert write_at (fromEnum (v1 `op` v2)) seq
...
perform_opcode_3 com modes = com
  { _sequence         = M.insert write_at first_input seq
 ...
 where
  ...
  write_at =
    get_position_by_mode seq instr_pos (_relative_base com) (head modes)
  first_input : other_inputs = _inputs com

perform_opcode_4 com modes = com { _current_position = pos + 2
...
perform_opcode_9 com modes = com { _relative_base    = _relative_base com + v
...
execute_current_opcode com =
  ...
  in  let pos = _current_position com
      in  let (instr, modes) = parse_opcode $ getVal pos seq
          in  case instr of
...
iterate_program c =
  let s = _sequence c
  in  let pos = _current_position c
      in  let (op, _) = parse_opcode $ getVal pos s
          in  case op of
                99        -> c { _state = Done }
                otherwise -> if (3 == op && _inputs c == [])
                  then c { _state = WaitingForInput }
                  else
                    let new_c = execute_current_opcode c
                    in  iterate_program $ new_c
...
runProgramWithInput input computer =
  let c = computer { _inputs = input : (_inputs computer) } in run_program c
